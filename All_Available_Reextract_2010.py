...
def weighted_means(z, name_list, dist_vars):    
    ...
    for i in sample_iter:
        ...
        with rasterio.open("ENP_Predictors/Final_Predictors_2010.tif") as src:
...
dist_vars = []
with open('Zebra_StepDist_Parameters.csv') as dist:
...
out_df = Parallel(n_jobs=6)(delayed(weighted_means)(z = q, name_list = name_list, dist_vars = dist_vars) for q in range(5, len(name_list[0])))
for w in range(5, len(name_list[0])):
    temp_df = out_df[w-5]
    temp_df.to_csv(''.join(['./', name_list[0][w], '_All_Available.csv']), sep=',',index=False)
